...
public class BarcodeEAN128Textdecoder {
  ...
  /**
   * Give back a list with all known EAN128 field descriptors
   */
  public static Map<String, BarcodeEAN128Field> getEAN128fieldlist() {

    Map<String, BarcodeEAN128Field> hash = new HashMap<>();
   ...
   * @param field the list of BarcodeEAN128Field, generated by getEAN128fieldlist
   * @return the converted string or null if there is a problem
   */
  public static String toHumanText(String text, Map<String, BarcodeEAN128Field> fields) {
    ...
    while (count < text.length()) {
      ...
      field = fields.get(text.substring(count, count + 2));
      if (field == null) {
        field = fields.get(text.substring(count, count + 3));
      }
      if (field == null) {
        field = fields.get(text.substring(count, count + 4));
      }
